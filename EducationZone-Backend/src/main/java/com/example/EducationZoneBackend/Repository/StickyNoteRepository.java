...
public interface StickyNoteRepository extends JpaRepository<StickyNote, Long> {
    ...
    @Query("SELECT new com.example.EducationZoneBackend.DTOs.StickyNoteDTOs.GetStickyNoteDTO(sn.id,sn.title,sn.description) FROM StickyNote sn JOIN Student s ON sn.student.id=s.id where sn.student.username=:username")
